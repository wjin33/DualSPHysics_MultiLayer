...
__global__ void kerInitializeVolFracRhoTauPstrain(unsigned n,unsigned pini,const typecode *code
  ,float2 *tauff,float2 *pstrain, float *tk, float4 *velrhop, float *VolFrac,TpVisco tvisco)
{
  ...
  if(p<n){
    ...
    if(pp1 == 0) // water phase
    {
      velrhop[p1].w = PHASESOILWATER[pp1].mw_rho;
      VolFrac[p1] = PHASESOILWATER[pp1].mw_VolFrac;
    }
    if(pp1 == 1) // soil phase
    {
      velrhop[p1].w = PHASESOILWATER[pp1].DP_rho;//Need to Stores constants for the GPU interaction
      VolFrac[p1] = PHASESOILWATER[pp1].DP_VolFrac;
      tk[p1] = 0.0f; // Initial plastic strain is set to zero
    }
  }
}

void InitializeVolFracRhoTauPstrain(unsigned np,unsigned npb,const typecode *code, tsymatrix3f *tau, tsymatrix3f *Pstraing, float *Tkg, float4 *Velrhopg,float *VolFracg, TpVisco tvisco, cudaStream_t stm)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetSimpleGridSize(npf,SPHBSIZE);
    kerInitializeVolFracRhoTauPstrain <<<sgridf,SPHBSIZE,0,stm>>> (npf,npb,code,(float2*)tau,(float2*)Pstraing,Tkg,(float4*)Velrhopg,VolFracg,tvisco);
  }
}
  ...
  if(p<n) {
    ...
    if(shift||arp1||acep1.x||acep1.y||acep1.z||visc||visceta||visco_etap1) {
      ...
      if(tvisco==VISCO_LaminarSPS) {
        float3 rg;
        rg=gradvelff[p1*3];		 rg=make_float3(rg.x+grap1_xx_xy.x,rg.y+grap1_xx_xy.y,rg.z+grap1_xz_yy.x);  gradvelff[p1*3]=rg;
        rg=gradvelff[p1*3+1];  rg=make_float3(rg.x+grap1_xx_xy.y,rg.y+grap1_xz_yy.y,rg.z+grap1_yz_zz.x);  gradvelff[p1*3+1]=rg;
        rg=gradvelff[p1*3+2];  rg=make_float3(rg.x+grap1_xz_yy.x,rg.y+grap1_yz_zz.x,rg.z+grap1_yz_zz.y);  gradvelff[p1*3+2]=rg;
      }
  ...
  if(p<n) {
    ...
    for(int c3=ini3; c3<fin3; c3+=nc.w)for(int c2=ini2; c2<fin2; c2+=nc.x) {
      ...
      if(pfin) {
        ...
        if (tvisco == VISCO_SoilWater){
        //    if (pp1 == 0){ // when pp1 is fluid, do nothing, needs interaction with boundary particles
  ...
  if(p<n) {
    ...
    if(tvisco == VISCO_SoilWater){
        ...
        if(pp1 == 1) {
          ...
          float2 Dpp1_yz_zz = make_float2(0,0);
          const float MC_AlphaPhi = 0.0;
          ////////////////
...
{
    // Add this check here
  if (pini > 13221 || pfin > 13221 || pfin < pini) {
    printf("CORRUPTION: p1=%u, pini=%u, pfin=%u, thread=%d, block=(%d,%d,%d)\n", 
           p1, pini, pfin, threadIdx.x, blockIdx.x, blockIdx.y, blockIdx.z);
    return;
  }
  for(int p2=pini; p2<pfin; p2++) {
    const float4 pscellp2=poscell[p2];
...
__device__ void KerInterpolationVelGranularToFluid(const unsigned& pini, const unsigned& pfin, const float4* poscell, const float4* velrhop
      , const typecode* code, const float4& pscellp1, float3& usp1)
  {
      ...
      usp1.x = usp1_numerator.x / usp1_denominator;
      usp1.y = usp1_numerator.y / usp1_denominator;
      usp1.z = usp1_numerator.z / usp1_denominator;
  }
      ...
      for (int p2 = pini; p2 < pfin; p2++) {
          ...
          if (rr2 <= CTE.kernelsize2 && rr2 >= ALMOSTZERO) {
              ...
              if (pp2 == 0) {// p1 is soil, p2 is fluid
                  ...
                  prep1_numerator += pressp2 * prep1_denominator; 
              }
          }
      }
...
__device__ void KerInteractionForcesMultilayerGranularBox_NN_SPH_PressGrad(bool boundp2, unsigned p1
      ...
      , float3& grap1_xx_xy_xz, float3& grap1_yx_yy_yz, float3& grap1_zx_zy_zz
      , float3& acep1, float& arp1, float& visc, float& deltap1
      , TpShifting shiftmode, float4& shiftposfsp1, float prep1,const float& volfracp1, float* volfrac)
  {
      for (int p2 = pini; p2 < pfin; p2++) {
          const float4 pscellp2 = poscell[p2];
          float drx = pscellp1.x - pscellp2.x + CTE.poscellsize * (CEL_GetX(__float_as_int(pscellp1.w)) - CEL_GetX(__float_as_int(pscellp2.w)));
          ...
          if (rr2 <= CTE.kernelsize2 && rr2 >= ALMOSTZERO) {
              ...
              massp2 = (boundp2 ? CTE.massb : PHASESOILWATER[pp2].mass);
              //Note if you masses are very different more than a ratio of 1.3 then: massp2 = (boundp2 ? PHASEARRAY[pp1].mass : PHASEARRAY[pp2].mass);
      ...
      , TpShifting shiftmode, float4& shiftposfsp1, float3& velsp1,const float& volfracp1, float* volfrac)
  {
      for (int p2 = pini; p2 < pfin; p2++) {
          ...
          if (rr2 <= CTE.kernelsize2 && rr2 >= ALMOSTZERO) {
              ...
              const typecode pp2 = (boundp2 ? pp1 : CODE_GetTypeValue(cod)); //<vs_non-Newtonian>
              float massp2; //massp2 not neccesary to go in _Box function
              massp2 = (boundp2 ? CTE.massb : PHASESOILWATER[pp2].mass); // for either soil or water, massp2 will be adusted automatically
              //Note if you masses are very different more than a ratio of 1.3 then: massp2 = (boundp2 ? PHASEARRAY[pp1].mass : PHASEARRAY[pp2].mass);
              ...
              if (compute){
                ...
                if (pp2 == 0) {
                  dvx_VolFrac = volfracp2 * velrhop2.x - volfracp1 * velrhop1.x;
                  dvy_VolFrac = volfracp2 * velrhop2.y - volfracp1 * velrhop1.y;
                  dvz_VolFrac = volfracp2 * velrhop2.z - volfracp1 * velrhop1.z;
                ...
                if (pp2 == 1) {//velsp1, velocity of solid particle at position p1
                    dvx_VolFrac = volfracp2 * velrhop2.x - (1 - volfracp1) * velsp1.x;
                    dvy_VolFrac = volfracp2 * velrhop2.y - (1 - volfracp1) * velsp1.y;
                    dvz_VolFrac = volfracp2 * velrhop2.z - (1 - volfracp1) * velsp1.z;
                  arp1 += velrhop1.w / volfracp1 * (USE_FLOATING ? ftmassp2 : massp2) / velrhop2.w * (dvx_VolFrac * frx + dvy_VolFrac * fry + dvz_VolFrac * frz);
                }
              }
              
              float cbar;
              if (tvisco != VISCO_SoilWater) cbar = max(PHASEARRAY[pp1].Cs0, PHASEARRAY[pp2].Cs0);
              else if (pp2 == 0) cbar = max(PHASEARRAY[pp1].Cs0, PHASEARRAY[pp2].Cs0);
              else cbar = 0;
              const float dot3 = (tdensity != DDT_None || shift ? drx * frx + dry * fry + drz * frz : 0);
              //-Density derivative (DeltaSPH Molteni).
              if (tdensity == DDT_DDT && deltap1 != FLT_MAX) {
  ...
  ,const float2 *sigma, float2 *rsigma)
{
  ...
  if(p<n) {
    ...
    for(int c3=ini3; c3<fin3; c3+=nc.w)for(int c2=ini2; c2<fin2; c2+=nc.x) {
      ...
      if(pfin) {
          if (tvisco != VISCO_SoilWater){
              KerInteractionForcesFluidBox_NN_SPH_PressGrad<tker, ftmode, tvisco, tdensity, shift, false>(false, p1, pini, pfin, ftomassp, poscell, velrhop, code, idp, CTE.massf, pp1, ftp1, pscellp1, velrhop1, pressp1, grap1_xx_xy_xz, grap1_yx_yy_yz, grap1_zx_zy_zz, acep1, arp1, visc, deltap1, shiftmode, shiftposfsp1);
              if (symm && rsymp1)	KerInteractionForcesFluidBox_NN_SPH_PressGrad<tker, ftmode, tvisco, tdensity, shift, true >(false, p1, pini, pfin, ftomassp, poscell, velrhop, code, idp, CTE.massf, pp1, ftp1, pscellp1, velrhop1, pressp1, grap1_xx_xy_xz, grap1_yx_yy_yz, grap1_zx_zy_zz, acep1, arp1, visc, deltap1, shiftmode, shiftposfsp1); //<vs_syymmetry>
          }
          ...
          if (tvisco == VISCO_SoilWater && pp1 == 1) {         //-If p1 is granular 
              prep1 = 0;
              KerInterpolationPFluidToGranular<tker, tvisco, false>(pini, pfin, poscell, velrhop, code, pscellp1, prep1, volfrac);
              if (symm && rsymp1) KerInterpolationPFluidToGranular<tker, tvisco, true>(pini, pfin, poscell, velrhop, code, pscellp1, prep1, volfrac);
              KerInteractionForcesMultilayerGranularBox_NN_SPH_PressGrad<tker, ftmode, tvisco, tdensity, shift, false>(false, p1, pini, pfin, ftomassp, poscell, velrhop, code, idp, CTE.massf, pp1, ftp1, pscellp1, velrhop1, pressp1, grap1_xx_xy_xz, grap1_yx_yy_yz, grap1_zx_zy_zz, acep1, arp1, visc, deltap1, shiftmode, shiftposfsp1, prep1, volfracp1, volfrac);
    ...
    for(int c3=ini3; c3<fin3; c3+=nc.w)for(int c2=ini2; c2<fin2; c2+=nc.x) {
      unsigned pini,pfin=0; cunsearch::ParticleRange(c2,c3,ini1,fin1,begincell,pini,pfin);
      if(pfin) {
        KerInteractionForcesFluidBox_NN_SPH_PressGrad<tker,ftmode,tvisco,tdensity,shift,false>(true,p1,pini,pfin,ftomassp,poscell,velrhop,code,idp,CTE.massb,pp1,ftp1,pscellp1,velrhop1,pressp1,grap1_xx_xy_xz, grap1_yx_yy_yz, grap1_zx_zy_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1);
        if(symm && rsymp1)	KerInteractionForcesFluidBox_NN_SPH_PressGrad<tker,ftmode,tvisco,tdensity,shift,true >(true,p1,pini,pfin,ftomassp,poscell,velrhop,code,idp,CTE.massb,pp1,ftp1,pscellp1,velrhop1,pressp1,grap1_xx_xy_xz, grap1_yx_yy_yz, grap1_zx_zy_zz,acep1,arp1,visc,deltap1,shiftmode,shiftposfsp1); //<vs_syymmetry>
      }
    }
...
#else	
  //(t.tvelgrad==VELGRAD_FDA) Interaction_ForcesGpuT_NN_FDA	    < tker,ftmode,tvisco,tdensity,shift>(t);
  //else if(t.tvelgrad==VELGRAD_SPH)	Interaction_ForcesGpuT_NN_SPH		< tker,ftmode,tvisco,tdensity,shift>(t);
  if(t.tvelgrad==VELGRAD_SPH)	Interaction_ForcesGpuT_NN_SPH		< tker,ftmode,tvisco,tdensity,shift>(t);
#endif
}
//==============================================================================
	...
	, const float3 *boundnormal, const float3 *motionvel, float4 *velrhop,tsymatrix3f *sigma)
{
	...
	if (p1<n) {
		...
		if (bnormalp1.x != 0 || bnormalp1.y != 0 || bnormalp1.z != 0) {
			...
			tsymatrix3f sigmap2 = { 0,0,0,0,0,0 };
